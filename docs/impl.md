...
- 生成数据覆盖 `29873` 个官方方块状态的红石导体属性和 `Full`, `Center`, `Rigid` 六方向支撑面.
...
- 漏斗受电时关闭. 未受电时依次尝试向朝向容器输出 1 件, 从上方容器拉取 1 件, 再吸收物品实体.
- 漏斗成功转移后进入 8 tick 冷却, 实现漏斗链的 7/8 tick 冷却修正.
